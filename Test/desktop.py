...
class RowLabel(Gtk.EventBox):
    ...
    def __init__(self, name_str):
        ...
        utilities.apply_css([self], self.css)
    ...
    def setup_signals(self):
        def _on_button_press_event(self, e):
            ...
            utilities.clipboard.set_text(self.value_label.get_label(), -1)
...
class TrashIcon(Icon):
    def __init__(self, icon_size):
        Icon.__init__(self, 'Trash', utilities.get_icon_from('user-trash', icon_size), icon_size)
...
class InfoWindow(Gtk.Window):
    ...
    def __init__(self):
        ...
        utilities.apply_alpha(self)
        utilities.apply_css([self], self.window_css)

        # Configure widgets
        utilities.apply_css([self.header_bar], self.header_bar_css)
        ...
        utilities.apply_css([self.separator], self.separator_css)
...
class Desktop(ResponsiveGrid):
    ...
    def _on_copy_path(self, item, e):
        ...
        utilities.clipboard.set_text(icon.path, len(icon.path))
        utilities.clipboard.store()
        return False

    def _on_paste(self, item, e):
        # Get the actual GLib file
        path = utilities.clipboard.wait_for_text()
        file = Gio.File.new_for_path(path)
    ...
    def register_menu_actions(self, menu):
        ...
        def _on_menu_open(menu, e, self):
            ...
            if self.selected_icon:
                ...
                if utilities.clipboard.wait_for_text() is not None:
    ...
    def grab_desktop_paths(self):
        ...
        while info is not None:
            ...
            if desktop_file.get_basename().endswith(".desktop"):
                ...
                icon_pixbuf = utilities.get_icon_from(icon_path, self.icon_size)
