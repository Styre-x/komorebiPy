...
class Settings:
    ...
    def load_settings():
        ...
        config_path = os.path.join(Settings.get_config_dir(), 'prop')
    ...
    def bootstrap_config_path():
        ...
        old_config_file = Gio.File.new_for_path(os.path.join(GLib.get_home_dir(), '.prop'))
        if old_config_file.query_exists():
            logging.info('Found config file from old version, converting it to new one...')
            destination_path = Gio.File.new_for_path(os.path.join(Settings.get_config_dir(), 'prop'))
            old_config_file.copy(destination_path, Gio.FileCopyFlags.NONE)

        Settings._config_file = Gio.File.new_for_path(os.path.join(Settings.get_config_dir(), 'prop'))
